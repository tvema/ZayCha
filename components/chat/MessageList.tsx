...
export function MessageList({
  ...
  const filteredMessages = messages.filter((m, index, self) => {
    let belongsToChat = false;
    if (activeGroup) belongsToChat = m.group_id === activeGroup.id;
    else if (activeContact) belongsToChat = (!m.group_id || m.group_id === 'null') && (m.sender_id === activeContact.id || m.receiver_id === activeContact.id);
  ...
  }, [chatId, activeContact, activeGroup]);
  ...
  useLayoutEffect(() => {
    ...
    if (unreadCountOnEnterRef.current > 0 && !firstUnreadMessageIdRef.current) {
      if (filteredMessages.length > 0 && (filteredMessages.length >= unreadCountOnEnterRef.current || !isLoadingMore)) {
        ...
        // Double check that we actually have the latest messages by comparing against last_message_timestamp
        const latestMsg = filteredMessages[filteredMessages.length - 1];
        const lastTimestamp = activeGroup ? activeGroup.last_message_timestamp : activeContact?.last_message_timestamp;
        
        let isUpToDate = true;
        if (lastTimestamp && latestMsg) {
          isUpToDate = new Date(latestMsg.created_at).getTime() >= new Date(lastTimestamp).getTime();
        }
