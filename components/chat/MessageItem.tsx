...
export function MessageItem({
  ...
  if (isJson) {
    try {
      ...
      if (parsedContent && !parsedContent.type && (parsedContent.url || parsedContent.fileId || parsedContent.fileUrl)) {
        parsedContent.type = 'file';
        if (parsedContent.fileUrl && !parsedContent.url) {
          parsedContent.url = parsedContent.fileUrl;
        }
        if (parsedContent.fileName && !parsedContent.name) {
          parsedContent.name = parsedContent.fileName;
        }
        if (parsedContent.fileType && !parsedContent.mime) {
          parsedContent.mime = parsedContent.fileType;
        }
        if (parsedContent.fileSize && !parsedContent.size) {
